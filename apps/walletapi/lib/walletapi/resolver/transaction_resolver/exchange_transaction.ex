defmodule WalletAPI.Resolver.TransactionResolver.ExchangeTransaction do
  ...
  def get_exchange_transaction(user_address,token, transfer_tx, transfers) do
    ...
    in_transfer = if (String.downcase(first_transfer[:from_address_hash]) == user_address) do first_transfer else second_transfer end
    out_transfer = if (String.downcase(first_transfer[:from_address_hash]) == user_address) do second_transfer else first_transfer end

    #Find the transfer related to the queried token
    [token_transfer] = Enum.filter([in_transfer,out_transfer], fn(event) ->
        event[:token] === token
    end)
    ...
    if(token_transfer == [])do
        ...
        implied_exchange_rate = cond do
            in_transfer[:token] == "cGLD" && out_transfer[:token] == "cUSD" -> %{"cGLD/cUSD" => Decimal.div(out_transfer[:value],in_transfer[:value])}
            out_transfer[:token] == "cGLD" && in_transfer[:token] == "cUSD" -> %{"cGLD/cUSD" => Decimal.div(in_transfer[:value],out_transfer[:value])}
        end

        value = Decimal.div(Decimal.mult(token_transfer[:value],if (token_transfer === in_transfer) do -1 else 1 end) , wei)
