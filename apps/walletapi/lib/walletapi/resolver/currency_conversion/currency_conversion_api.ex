defmodule WalletApi.CurrencyConversion.CurrencyConversionAPI do
  ...
  defp get_supported_exchange_rate(from_code, to_code, timestamp, implied_exchange_rate) do
    ...
    cond do
      implied_exchange_rate && implied_exchange_rate[pair] ->
        implied_exchange_rate[pair]

      pair === "cUSD/USD" || pair === "USD/cUSD" ->
        Decimal.from_float(1.0)

      pair === "cGLD/cUSD" || pair === "cUSD/cGLD" ->
        GoldExchangeRateAPI.get_exchange_rate(from_code, to_code, timestamp)

      true ->
        ExchangeRateAPI.get_exchange_rate(from_code, to_code, timestamp)
    end
  end
end
