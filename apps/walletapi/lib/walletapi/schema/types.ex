defmodule WalletApi.Schema.Types do
    ...
    defp resolve_transaction_type(obj, _) do
      if(obj.type == :exchange) do
        ...
        obj.type == :verification_reward) do
          :token_transfer
        end
      end
    end
    ...
    defp resolve_local_amount(money_amount, _,%{context: %{local_currency_code: local_currency_code}}) do
      ...
      args = %{:source_currency_code => money_amount[:currency_code],
      ...
      local_amount = %{:value => Decimal.mult(money_amount[:value],rate), :currency_code => local_currency_code , :exchange_rate => rate}
      {:ok, local_amount}
    end
end
